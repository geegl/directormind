...
Status: `NEW_P1_REPAIR_LOCAL_PASS / FINAL_CI_AND_INDEPENDENT_REVIEW_PENDING`
...
| Current repository unit/CLI behavior | `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s skills/drama-director-compiler/tests -v` | Nine versioned test modules | PASS — 233 tests | Covers the current repository test surface; not source replay or audience evaluation |
...
| Director IR compatibility and upgrade boundary | `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s skills/drama-director-compiler/tests -p 'test_director_*.py' -v` | Canonical-input replay, scene/fact/Shot binding, v0.2 legacy-mode rejection, result-only/input-plus-result/same-case forgery attacks, routed positive path, input/symlink/existing-output guards, exclusive creation and read-only check mode | PASS — 55 tests | Proves local contract behavior on synthetic/fixture IR; final CI and clean-checkout independent replay remain pending |
...
| Minimal CI definition | `.github/workflows/directormind-contracts.yml` | Read-only workflow calling the complete local runner | PENDING_CURRENT_HEAD | Earlier heads passed; the new local P1 repair is not yet pushed and no prior run proves it |
...
Historical audit verdicts remain evidence for their reviewed snapshots only. A new independent review reproduced two further P1s: `LEGACY_COMPATIBLE` accepted an unbound v0.2 route, and the routing CLI overwrote inputs, symlink aliases and arbitrary existing output. Both are locally repaired with 55 targeted tests, 233 complete tests and 18 checks passing. `FINAL_INTEGRATION_INDEPENDENT_AUDIT.md` records the superseding FAIL state. No current `NO_MUST_FIX_FINDINGS` claim is allowed until final-head CI and a fresh clean-checkout replay pass.
