...
| Current repository unit/CLI behavior | `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s skills/drama-director-compiler/tests -v` | Nine versioned test modules | PASS — 225 tests | Covers the current repository test surface; not source replay or audience evaluation |
...
| Director IR compatibility and upgrade boundary | `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s skills/drama-director-compiler/tests -p 'test_director_*.py' -v` | Canonical enum/sentinel behavior, canonical-input replay, scene/fact/Shot binding, cross-scene substitution rejection, malformed-type rejection, version-scoped legacy GO triggers, two upgrade modes, all-existing-output non-overwrite guard, audio validator/renderer regressions | PASS — 47 tests | Proves local contract behavior on synthetic/fixture IR; does not migrate or modify private Director IR and does not make legacy audio executable |
