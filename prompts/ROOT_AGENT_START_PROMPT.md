...
- 审计现有覆盖图；
- 完整读取并执行 `research/coverage/FIRST_16_SCENES.md`，在这 16 个场景完成前不自行缩减首批范围或选择第二批；
- 核验每个场景的季、集、本地版本时间段及合法取材方式；
- 本机已有视频的场景直接逐个完成；缺失场景统一维护一次性素材清单，不虚构镜头事实，也不因缺失项停止其他可用场景；
