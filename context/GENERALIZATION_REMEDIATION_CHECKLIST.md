...
DirectorMind evidence generalization phase 1: audit the existing 30 local-source evidence files and establish the remediation checklist, Scene Evidence schema, validator, and minimum validator tests.
...
- Existing 30 evidence Markdown files remain unchanged during this phase.
...
- [x] The repository validator implements every B1 gate planned for canonical JSON.
- [x] At least the ten B2 positive/negative cases pass, including the CLI path.
...
- Current sole implementer/writer: `/root`; three sub-agents performed read-only 10/10/10 audits.
- Completed in this phase commit: checklist, 30-file audit report, Scene Evidence schema, standard-library validator, and 45 validator tests.
- Not completed: canonical conversion of the 30 files and all later A3–M work.
- Validation result: schema JSON syntax, Python compile, and 45 unit/CLI tests pass; the third frozen snapshot received independent PASS with no scoped P0/P1/P2.
...
- Next single action: push this phase commit to PR #3, then stop and wait for the next explicit phase instruction. Do not start A3 now.

Rollback for this phase: before merge, revert the single phase commit or remove only the five newly introduced artifacts; no legacy evidence file is changed. After merge, use a normal Git revert commit and new pull request. Do not rewrite shared history.
...
- `python3 -m unittest discover -s skills/drama-director-compiler/tests -v` — PASS, 45 tests.
...
- The third frozen-snapshot adversarial review — PASS: no scoped P0, P1, or P2 remained.
...
| ID | Status | Requirement | Evidence / exit condition |
|---|---|---|---|
| B1 | VERIFIED_DONE | Add `validate_scene_evidence.py` covering schema, shot identity/timing, provenance, UNKNOWN leakage, rule references/boundaries, HIGH-risk fallback, surface-copy guards, and public-repository prohibitions. | Standard-library validator plus three frozen independent adversarial reviews; final verdict PASS. |
| B2 | VERIFIED_DONE | Add minimum tests: valid evidence, gap, overlap, missing Shot ref, UNKNOWN promotion, HIGH without fallback, missing non-applicability, single-source GENERAL_DEFAULT, audio rule without observed audio, and reference-surface leakage. | 45/45 unit and CLI tests pass, including all required cases and adversarial equivalents. |
...
Commit and push the independently approved P3/A1/A2/B1/B2 phase-1 snapshot to PR #3, then stop. Do not begin A3 without the next explicit phase instruction.
